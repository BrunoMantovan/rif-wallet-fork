...
export default function OrderTaken({ route, navigation }) {
  ...
  const isAssetERC20 = asset => asset.decimals !== undefined
  const assets = balances.filter(b => isAssetERC20(b))
  const { escrowToken, releaseToken } = usePaymentExecutor(assets[0])
    ...
    <View
      ...
      <View style={{ flex: 5, justifyContent: 'center', alignItems: 'center' }}>
        ...
        <View style={{ justifyContent: 'center', alignItems: 'center' }}>
          <Icon name={'check-circle-o'} size={150} color={'#19AD79'} />
          <View
            style={{
              flexDirection: 'row',
            ...
            <Text style={[styles.mainText, { fontSize: 30 }]}>
              {formatNumberWithDots(order.amount)}{' '}
            </Text>
            <Text style={[styles.mainText, { fontSize: 30 }]}>
              {order.tokenCode}
            </Text>
          ...
          <Text
            ...
            Tus crypto fueron depositadas en tu cuenta exitosamente
          </Text>
        </View>
      </View>
    ...
    <ScrollView
      ...
      <View style={{ flex: 1 }}>
        <View style={styles.total}>
          ...
          <Text
            style={[styles.mainText, { fontSize: 18, letterSpacing: 0.25 }]}>
            {order.tokenCode}
          </Text>
        </View>
        <View style={{ marginBottom: 8 }}>
          <View
            ...
            <Text
              style={[
                styles.mainText,
                { fontSize: 16, letterSpacing: 0.5, color: '#B0B3B5' },
              ]}>
              Precio por unidad
            </Text>
            <Text style={[styles.mainText, { fontSize: 20 }]}>
              ${formatNumberWithDots(order.fiatAmount / order.amount)} ARS
            </Text>
          </View>
          <View
            style={{
            ...
            <Text style={[styles.mainText, { fontSize: 20 }]}>
              ${formatNumberWithDots(order.fiatAmount)} ARS
            </Text>
          </View>
          <View
            style={{
          ...
          <View
            ...
            <Text
              ...
              {order.type == 'SELL' ? 'Vendedor' : 'Comprador'}
            </Text>
            <Text style={[styles.mainText, { fontSize: 20 }]}>
              {order.username}
            </Text>
          ...
          <View
            style={{
              borderBottomWidth: 1,
              width: '100%',
              borderBottomColor: sharedColors.bagreen,
              marginVertical: 24,
            }}
          />
        </View>
        {order.type === 'SELL' && userInfo.id !== order.creatorId ? (
          <SellNotCreator
            status={order.status}
            onPress={() => orderUpdate('FIAT_SENT')}
          />
        ) : order.type === 'SELL' && userInfo.id === order.creatorId ? (
          <SellCreator
            status={order.status}
            fiatAmount={order.fiatAmount}
            onLock={handleApprove}
