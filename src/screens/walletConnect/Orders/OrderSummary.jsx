...
export default function OrderSummary({ route, navigation }) {
  ...
  async function handleSubmit() {
    try {
      ...
      tokenCode,
      fiatAmount: calculateARSEquivalent().toString(),
      fiatCode: "ARS",
      walletAddress: orderType === "BUY" ? walletAddress : undefined,
      paymentMethods: [
        {
          type: "FINTECH",
          alias: username,
      ...
      */
      ...
      const newOrder = {
        ...
        fiatCode: 'ARS',
        walletAddress:
          order.order_type == 'BUY' ? wallet.smartAddress : undefined,
  ...
  return isAddressLoading ? (
    ...
    <View style={{ padding: 16, flex: 1 }}>
      <Text style={styles.text}>
        Tipo de operación:{' '}
        <Text style={styles.innetText}>{order.order_type_for_self}</Text>
      </Text>
      <Text style={styles.text}>
        Activo: <Text style={styles.innetText}>{order.crypto}</Text>
      </Text>
      <Text style={styles.text}>
        Moneda de {order.order_type_for_self == 'Comprar' ? 'compra' : 'cobro'}:{' '}
        <Text style={styles.innetText}>ARS</Text>
      </Text>
      <Text style={styles.text}>
        Precio unitario: <Text style={styles.innetText}>${order.price}</Text>
      </Text>
      <Text style={styles.text}>
        Cantidad total:{' '}
        <Text style={styles.innetText}>
          {order.total} {order.crypto}
        </Text>
      </Text>
      {order.order_type == 'Comprar' ? (
        <Text style={styles.text}>
          Método de pago:{' '}
          <Text style={styles.innetText}>
            {order.payment_method.entity +
              ' (' +
              order.payment_method.alias +
              ')'}
          </Text>
        </Text>
      ) : null}
      <Text style={styles.text}>
        Billetera:{' '}
        <Text style={styles.innetText}>
          {shortAddress(wallet.smartAddress, 10)}
        </Text>
      </Text>
