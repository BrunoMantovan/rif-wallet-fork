...
export default function CreateOrder() {
  ...
  const cryptos = [
    ...
    { text: 'tRIF', image: require('../../../images/slides/doc.png') },
  ]
    ...
    <GestureHandlerRootView style={{ flex: 1 }}>
      <ScrollView style={styles.body}>
        <View
          ...
          <Pressable
            onPress={() => selectType('Comprar')}
            style={[
              styles.orderSelector,
              type === 'BUY' ? styles.selectedOrder : null,
              { borderTopLeftRadius: 8, borderBottomLeftRadius: 8 },
            ]}
            android_ripple={{
              borderless: false,
              foreground: true,
              color: sharedColors.balightblue1,
            }}>
            <Text
              style={[
                styles.orderText,
                type === 'BUY' ? styles.selectedText : null,
              ]}>
              Comprar
            </Text>
          </Pressable>
          <Pressable
            onPress={() => selectType('Vender')}
            style={[
              styles.orderSelector,
              type === 'SELL' ? styles.selectedOrder : null,
            ...
            <Text
              style={[
                styles.orderText,
                type === 'SELL' ? styles.selectedText : null,
              ]}>
              Vender
            </Text>
          </Pressable>
          ...
          <View style={{ flexDirection: 'row', marginBottom: 14 }}>
            <View style={{ height: 100, width: '50%', paddingRight: '2%' }}>
              ...
              <Dropdown
                ...
                onPress={() => toggleOpen(null, 2)}
                image={
                  cryptoPlaceholder === 'rBtc'
                    ? require('../../../images/slides/rbtc.png')
                    : require('../../../images/slides/doc.png')
                }
              />
            </View>
            <View style={{ height: 100, width: '50%', paddingRight: '2%' }}>
              ...
              <Dropdown
                placeholder="ARS"
                image={require('../../../images/slides/rbtc.png')}
              />
            </View>
          </View>
        </View>
        <View
          style={{ width: '100%', alignContent: 'center', marginBottom: 14 }}>
          <Text style={styles.subtitle}>CANTIDAD TOTAL</Text>
          <Text style={styles.span}>
            ¿Cuánto {crypto} querés {type === 'BUY' ? 'comprar' : 'Vender'}?
          </Text>
          <InputText
            value={total}
            setValue={value => handleNumberChange(value, 2)}
            placeholder="0"
            keyboard="numeric"
            style={styles.input}
          />
        ...
        <View
          ...
          <Text style={styles.span}>
            ¿A qué precio querés {type === 'BUY' ? 'comprar' : 'Vender'}{' '}
            {crypto}?
          </Text>
          <InputText
            value={price}
            setValue={value => handleNumberChange(value, 1)}
            placeholder="0"
            keyboard="numeric"
            style={styles.input}
          />
        ...
        <View style={{ flex: 1, justifyContent: 'flex-end', marginBottom: 24 }}>
          <ButtonCustom
            onPress={specs ? onPressing : undefined}
            text="Continuar"
            type={specs ? 'green' : 'disabled'}
            activeOpacity={specs ? false : 1}
            icon="arrow-right"
          />
        </View>
      </ScrollView>
      {open && (
        <>
          <AnimatedPressable
            style={[styles.backdrop, { zIndex: 3 }]}
            entering={FadeIn}
            exiting={FadeOut}
            onPress={() => toggleOpen(null)}
          />
          <BottomSheet
            data={data}
